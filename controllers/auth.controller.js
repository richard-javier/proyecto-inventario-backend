...
export const registrarUsuario = async (req, res) => {
    const { nombre, apellido, cedula, correo_electronico, contrasena, id_rol } = req.body;

    if (!correo_electronico || !contrasena || !id_rol) {
        return res.status(400).json({ message: 'Faltan campos obligatorios (correo, contraseña, rol).' });
    }
...
export const loginUsuario = async (req, res) => {
    ...
    if (!correo_electronico || !contrasena) {
        return res.status(400).json({ message: 'Debe ingresar correo electrónico y contraseña.' });
    }
    ...
    try {
        ...
        const [users] = await connection.query(
            'SELECT id_usuario, contrasena_hash, id_rol, nombre_rol FROM usuarios u JOIN roles r ON u.id_rol = r.id_rol WHERE correo_electronico = ?',
            [correo_electronico]
        );
